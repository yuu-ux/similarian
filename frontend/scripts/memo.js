...
window.setupMemo = function() {
    ...
    memoData.forEach(memo => {
        ...
        memoItem.innerHTML = `
            ...
            <div class="memo-item-group-button">
                <div class="memo-item-button">
                    <button class="edit-group" onclick="showGroupEditModal(event, ${memo.id})">グループ編集</button>
                </div>
            </div>
        `;
...
// グループ編集モーダルを表示する関数
function showGroupEditModal(event, memoId) {
    event.stopPropagation(); // イベントの伝播を停止
    
    const existingModal = document.querySelector('.group-edit-modal');
    if (existingModal) {
    ...
    // モーダルの内容を設定
    modal.innerHTML = `
        <input type="text" placeholder="新しいグループを追加" class="group-name-input">
        <div class="existing-memo-list">
            <div class="group-select-item">
                <input type="checkbox" id="group1" value="グループ1">
                <label for="group1">グループ1</label>
            </div>
            <div class="group-select-item">
                <input type="checkbox" id="group2" value="グループ2">
                <label for="group2">グループ2</label>
            </div>
            <div class="group-select-item">
                <input type="checkbox" id="group3" value="グループ3">
                <label for="group3">グループ3</label>
            </div>
        </div>
        <div class="group-edit-modal-button">
            <button onclick="updateGroup(${memoId})">更新</button>
            <button onclick="deleteGroup(${memoId})" class="delete-button">削除</button>
        </div>
    `;

    const groupItems = modal.querySelectorAll('.group-select-item');
...
// グループを更新する関数
function updateGroup(memoId) {
    const input = document.querySelector('.group-name-input');
    const newGroupName = input.value.trim();
    const selectedGroups = Array.from(document.querySelectorAll('.group-select-item input[type="checkbox"]:checked'))
        .map(checkbox => checkbox.value);
    
    if (newGroupName || selectedGroups.length > 0) {
        console.log(`メモID: ${memoId}のグループを更新:`, {
            newGroup: newGroupName,
            selectedGroups: selectedGroups
        });
        
        const modal = document.querySelector('.group-edit-modal');
        if (modal) {
            modal.remove();
        }
    }
